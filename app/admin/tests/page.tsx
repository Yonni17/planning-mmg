...
export default function TestsAdminPage() {
  ...
  async function sendEmail(template: 'opening'|'weekly'|'deadline'|'planning') {
    ...
    try {
      ...
      const res = await fetch('/api/admin/email-test', {
        ...
        body: JSON.stringify({ to: testEmail, template, period_id: periodId || null }),
      });
      const json = await res.json();
      ...
      <div className="rounded-xl border border-zinc-700 bg-zinc-900/60 p-4 space-y-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <input
            ...
            className="border border-zinc-700 rounded bg-zinc-900 text-zinc-100 px-3 py-1 w-64"
          />
          <button onClick={() => sendEmail('opening')} disabled={!testEmail || sending.opening} className="rounded bg-zinc-800 hover:bg-zinc-700 text-zinc-100 text-sm px-3 py-1.5 disabled:opacity-50">
            {sending.opening ? '…' : 'Envoyer “Ouverture”'}
          </button>
          <button onClick={() => sendEmail('weekly')} disabled={!testEmail || sending.weekly} className="rounded bg-zinc-800 hover:bg-zinc-700 text-zinc-100 text-sm px-3 py-1.5 disabled:opacity-50">
            {sending.weekly ? '…' : 'Envoyer “Hebdo”'}
          </button>
          <button onClick={() => sendEmail('deadline')} disabled={!testEmail || sending.deadline} className="rounded bg-zinc-800 hover:bg-zinc-700 text-zinc-100 text-sm px-3 py-1.5 disabled:opacity-50">
            {sending.deadline ? '…' : 'Envoyer “48/24/1h”'}
          </button>
          <button onClick={() => sendEmail('planning')} disabled={!testEmail || sending.planning} className="rounded bg-zinc-800 hover:bg-zinc-700 text-zinc-100 text-sm px-3 py-1.5 disabled:opacity-50">
            {sending.planning ? '…' : 'Envoyer “Planning prêt”'}
          </button>
        ...
        <p className="text-xs text-zinc-400">Astuce : pour tester les liens, configure bien Resend et <code>PLANNING_FROM_EMAIL</code>.</p>
      </div>
