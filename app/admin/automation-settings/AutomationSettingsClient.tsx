...
export default function AutomationSettingsClient() {
  ...
  async function loadSettings() {
    ...
    try {
      ...
      const data = await res.json(); // { scope, period_id, effective, raw, global }
      ...
      if (raw) {
        ...
        // Pas d’override → on garde la période sélectionnée et les valeurs par défaut
        setSettings((s) => ({ ...s, period_id: periodId || null }));
      }
    } catch (e: any) {
  ...
  async function saveSettings() {
    ...
    setMsg('');
    try {
      const body: Settings = { ...settings, period_id: periodId || null };
