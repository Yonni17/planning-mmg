...
// --- Helpers semaine vs week-end/férié (FRONT ONLY) ---
const WEEKEND_KINDS: SlotKind[] = ['SAT_12_18', 'SAT_18_00', 'SUN_08_14', 'SUN_14_20', 'SUN_20_24'];
const isWeekendKind = (k?: SlotKind | null) => !!k && WEEKEND_KINDS.includes(k);
    ...
    if (result) {
      for (const a of result.assignments) {
        ...
        if (isWeekendKind(a.kind)) {
          ...
          weekCnt.set(uid, (weekCnt.get(uid) ?? 0) + 1);
        }
      }
    }
            ...
            <tbody>
              {doctorRows.map((r, idx) => (
                <tr key={`${r.user_id}-${idx}`} className="border-t">
                  ...
                  <td className="p-2">{r.assignedWeek}</td>
                  <td className="p-2">{r.assignedWeekend}</td>
